...
details, `TESTING.md` for the full test strategy, and `docs/core/SUPPORT_TIERS.mbx.md` for crate and
...
```bash
cargo xtask pre-commit   # fmt/restage staged Rust, clippy, architecture, docs/date checks
cargo xtask test unit    # run workspace library tests (any platform)
...
# Pre-commit gate (may format and re-stage tracked staged Rust files)
cargo xtask pre-commit
# Pre-push gate (musl-check + release build + release nextest + conformance)
cargo xtask prepush
```
...
```bash
just test-e2e                # protocol e2e tests (any platform)
just test-system             # full daemon+CLI suite (Linux + root)
just test-e2e-vps            # run full-stack suite on VPS via SSH
```
...
`pre-commit` runs `cargo fmt --all` when Rust is staged, re-stages tracked Rust changes,
runs targeted clippy plus the architecture guard, lints staged agent/workflow files, lints docs,
and refreshes the feature-matrix date. `prepush` starts with `cargo xtask musl-check`, then runs
the release build, release library tests, and conformance. GitHub workflows cover the active
`develop` → `staging` → `release` → `main` promotion path.
...
| ---------------------------- | ------------------------ | --------------------------------------------------------- |
| Before every commit          | `cargo xtask pre-commit` | conditional fmt/restage + clippy/architecture + docs/date |
| Before every push            | `cargo xtask prepush`    | musl-check + release build/tests + conformance            |
| Read-only local verification | `cargo xtask verify`     | fmt/check/clippy + architecture/borrow/docs               |
...
| Task                              | Command                         | When to use                             |
| --------------------------------- | ------------------------------- | --------------------------------------- |
| Unit library tests                | `cargo xtask test unit`         | Any platform; no root required          |
...
| Lint legacy plan/spec frontmatter     | `cargo xtask docs lint`                     | Only scans docs/superpowers/        |
| Enforce dependency rings              | `cargo xtask architecture`                  | After changing crate dependencies   |
| Cross-build release binaries for musl | `cargo xtask musl-check`                    | Before pushing Linux-gated changes  |
...
| Host-architecture musl binaries | `just build-linux`                          | Linux target matching the build host |
| x86_64 musl deployment check    | `cargo xtask musl-check`                    | Verify binaries for the x86_64 VPS   |
| Build + load test image         | `cargo xtask build-test-image`              | Required before `test-linux`         |
| Build CLI with TUI              | `cargo build -p minibox-cli --features tui` | Enables `mbx tui`                    |
...
These are **not** part of the core build/test pipeline. See `just --list`
for recipes that wrap them.
