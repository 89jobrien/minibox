...
**Crates touched:** `minibox` (tests + code change)
...
2. The `as_any!`, `default_new!`, and `adapt!` macros (defined in `minibox-macros`, used in `minibox`) have no tests documenting downcast contracts or failure modes.
...
**File:** `crates/minibox/src/image/layer.rs`, existing `#[cfg(test)]` block.
...
**File:** `crates/minibox/src/adapters/mocks.rs`, new `#[cfg(test)]` block at bottom.

The `as_any!` macro body uses `crate::domain::AsAny`. Because `macro_rules!` path resolution happens at the call site, `crate` resolves to whichever crate expands the macro — in this case `minibox`. `minibox-macros` itself has no `domain` module, so tests must live in `minibox`. The mock adapters (`MockRegistry`, `MockFilesystem`, `MockLimiter`, `MockRuntime`) already use `adapt!`, making `mocks.rs` the natural test site.
...
**File:** `crates/minibox/src/image/registry.rs`, `pull_image` method.
