compile compco# macbox + daemonbox Design Spec
...
├─────────────────────────────────────────────────────┤
│              Driven Adapters                         │
│  mbx/src/adapters/ (Linux, GKE, Colima)           │
...
**Dependencies:** `mbx`, `tokio`, `serde_json`, `tracing`, `nix` (POSIX subset: signal, wait, socket), `uuid`, `chrono`, `anyhow`
...
**Dependencies:** `daemonbox` (for `HandlerDependencies`), `mbx` (for Colima adapter types), `anyhow`, `thiserror`, `tokio`, `tracing`
...
```
miniboxd    ──► daemonbox  ──► mbx
macboxd     ──► daemonbox  ──► mbx
macboxd     ──► macbox     ──► daemonbox
                               mbx
minibox-cli ──► mbx        (unchanged)
```
