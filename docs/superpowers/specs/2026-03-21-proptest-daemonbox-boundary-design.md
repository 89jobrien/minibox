...
Wire `HandlerDependencies` using `adapters::mocks` (already present in minibox). No Linux syscalls are required. Both `handle_stop` and `handle_remove` check `state.get_container(id)` first — for an unknown ID they return `ContainerNotFound` before touching any filesystem or mock dependencies.
...
## File 2: `crates/minibox/tests/proptest_suite.rs` (extend)
...
- Extensions to existing minibox property suite → `crates/minibox/tests/proptest_suite.rs`
