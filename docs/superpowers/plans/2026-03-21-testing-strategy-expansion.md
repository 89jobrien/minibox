...
New test files:
- `crates/minibox-lib/tests/adapter_native_tests.rs` — native adapter tests (cgroups, overlay, namespace setup)
- `crates/minibox-lib/tests/adapter_gke_tests.rs` — GKE adapter tests (proot, copy FS)
- `crates/minibox-lib/tests/adapter_colima_tests.rs` — Colima adapter tests (Colima VM specifics)
- `crates/minibox-lib/src/adapters/test_fixtures.rs` — Shared test fixtures, mock builders
- `crates/daemonbox/tests/handler_adapter_swap_tests.rs` — Handler tests with mock adapter swaps

Modified files:
- `crates/minibox-lib/src/adapters/mocks.rs` — Extend mocks with failure injection helpers
- `crates/minibox-lib/src/adapters/mod.rs` — Export test_fixtures module

### Phase 2: Property-Based Testing
New test files:
- `crates/minibox-lib/tests/protocol_codec_properties.rs` — Extended codec roundtrip + edge cases
- `crates/minibox-lib/tests/path_validation_properties.rs` — Path traversal, symlinks, normalization
- `crates/minibox-lib/tests/image_manifest_properties.rs` — Manifest parsing invariants
- `crates/minibox-lib/tests/cgroup_boundary_properties.rs` — Resource limit boundaries, overflow handling

Modified files:
- `crates/minibox-lib/tests/proptest_suite.rs` — Consolidate existing + new properties into suites

### Phase 3: Chaos & Fault Injection
New test files:
- `crates/minibox-lib/tests/adapter_failure_injection_tests.rs` — Adapter failures (incomplete pulls, cgroup errors)
...
**Files:**
- Create: `crates/minibox-lib/tests/adapter_native_tests.rs`
...
Run: `cargo test -p minibox-lib --test adapter_native_tests -- --nocapture`
Expected: FAIL — function `create_native_filesystem_adapter` not found

- [ ] **Step 3: Implement native adapter test fixture**

In `crates/minibox-lib/tests/adapter_native_tests.rs`:

```rust
use minibox_lib::adapters::native_filesystem::NativeFilesystemProvider;
...
```bash
git add crates/minibox-lib/tests/adapter_native_tests.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/adapter_gke_tests.rs`
...
Run: `cargo test -p minibox-lib --test adapter_gke_tests -- --nocapture`
Expected: FAIL — function `create_gke_filesystem_adapter` not found

- [ ] **Step 3: Implement GKE adapter test fixture**

In `crates/minibox-lib/tests/adapter_gke_tests.rs`:

```rust
use minibox_lib::adapters::gke::GkeFilesystemProvider;
use minibox_lib::domain::FilesystemProvider;
use std::path::Path;
use std::sync::Arc;
...
```bash
git add crates/minibox-lib/tests/adapter_gke_tests.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/adapter_colima_tests.rs`
...
Run: `cargo test -p minibox-lib --test adapter_colima_tests -- --nocapture`
Expected: FAIL

- [ ] **Step 3: Implement Colima adapter test fixture with mocked sysfs**

In `crates/minibox-lib/tests/adapter_colima_tests.rs`:

```rust
use minibox_lib::adapters::colima::ColimaResourceLimiter;
use minibox_lib::domain::ResourceLimiter;
use std::sync::Arc;
...
```bash
git add crates/minibox-lib/tests/adapter_colima_tests.rs
...
**Files:**
- Create: `crates/minibox-lib/src/adapters/test_fixtures.rs`
...
In `crates/minibox-lib/src/adapters/test_fixtures.rs`:
...
In `crates/minibox-lib/src/adapters/mod.rs`, add:
...
Run: `cargo test -p minibox-lib adapters::test_fixtures -- --nocapture`
Expected: PASS

- [ ] **Step 4: Commit**

```bash
git add crates/minibox-lib/src/adapters/test_fixtures.rs crates/minibox-lib/src/adapters/mod.rs
...
In `crates/minibox-lib/src/adapters/mocks.rs`, add after existing mocks:
...
Run: `cargo test -p minibox-lib adapters::mocks -- --nocapture`
Expected: PASS
...
async fn test_handler_run_container_with_mock_filesystem_adapter() {
    ...
    use minibox_lib::adapters::test_fixtures::MockAdapterBuilder;
...
```bash
cargo test -p minibox-lib --test adapter_native_tests
cargo test -p minibox-lib --test adapter_gke_tests
...
Create `crates/minibox-lib/src/adapters/ADAPTER_CONTRACT.md`:
...
```bash
git add crates/minibox-lib/src/adapters/ADAPTER_CONTRACT.md Justfile
...
**Files:**
- Create: `crates/minibox-lib/tests/protocol_codec_properties.rs`
...
use minibox_lib::protocol::{DaemonRequest, encode_request, decode_request};
...
Run: `cargo test -p minibox-lib --test protocol_codec_properties -- --nocapture`
Expected: PASS (hundreds of test cases)

- [ ] **Step 5: Commit**

```bash
git add crates/minibox-lib/tests/protocol_codec_properties.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/path_validation_properties.rs`
...
Run: `cargo test -p minibox-lib --test path_validation_properties -- --nocapture`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add crates/minibox-lib/tests/path_validation_properties.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/image_manifest_properties.rs`
...
```rust
use proptest::prelude::*;
use minibox_lib::image::manifest::{OciManifest, parse_manifest, encode_manifest};
...
Run: `cargo test -p minibox-lib --test image_manifest_properties -- --nocapture`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add crates/minibox-lib/tests/image_manifest_properties.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/cgroup_boundary_properties.rs`
...
```rust
use proptest::prelude::*;
use minibox_lib::container::cgroups::{validate_memory_limit, set_memory_limit};
...
Run: `cargo test -p minibox-lib --test cgroup_boundary_properties -- --nocapture`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add crates/minibox-lib/tests/cgroup_boundary_properties.rs
...
```bash
cargo test -p minibox-lib --test protocol_codec_properties
cargo test -p minibox-lib --test path_validation_properties
cargo test -p minibox-lib --test image_manifest_properties
cargo test -p minibox-lib --test cgroup_boundary_properties
```
...
Add to `crates/minibox-lib/tests/proptest_suite.rs`:
...
```bash
git add crates/minibox-lib/tests/proptest_suite.rs
...
**Files:**
- Create: `crates/minibox-lib/tests/adapter_failure_injection_tests.rs`
...
async fn test_adapter_handles_incomplete_layer_download_with_cleanup() {
    use minibox_lib::adapters::mocks::FailableImageRegistryMock;
...
async fn test_limiter_handles_cgroup_creation_failure() {
    use minibox_lib::adapters::mocks::FailingResourceLimiterMock;
...
async fn test_filesystem_adapter_cleans_up_layers_on_mount_failure() {
    use minibox_lib::adapters::mocks::FailableFilesystemMock;
...
Run: `cargo test -p minibox-lib --test adapter_failure_injection_tests -- --nocapture`
Expected: PASS

- [ ] **Step 5: Commit**

```bash
git add crates/minibox-lib/tests/adapter_failure_injection_tests.rs
...
```bash
cargo test -p minibox-lib --test adapter_failure_injection_tests
