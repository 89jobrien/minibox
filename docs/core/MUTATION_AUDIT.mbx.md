...
| Canonical-parent escape check (`!canonical_parent.starts_with(&canonical_dest)`) | No test exercises a symlink-crafted escape through `parent.canonicalize()` | FAIL    |
...
| Guard / step                                                     | Test                                                                                                                                    | Verdict |
| ---------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- | ------- |
| Default socket mode `0o600` applied via `set_permissions`        | No test verifies that the socket file is created with mode `0o600`. The code path is exercised only in the daemon binary main function. | FAIL    |
...
_Note: the three GHCR size-limit failures share a single root cause — the GHCR
adapter's wiremock test suite does not have an oversized-response scenario,
unlike `registry.rs` which has `get_manifest_errors_when_content_length_exceeds_limit`._
