...
- `tailbox` crate — `TailnetNetwork` adapter, `TailnetConfig`, auth key resolution
  ...
  _(moved to `minibox-plugins` workspace; not present in this workspace)_
...
- Multi-turn `infer()` API, `Message`/`ContentBlock` types, Ollama auto-detection
  ...
  _(dropped in Phase 0 consolidation — orphan crate, not carried forward)_
...
- Three-tier git workflow: `main` → `next` (auto) → `staging` (manual)
...
- Three-tier git workflow — `main` → `next` (auto) → `staging` (manual dispatch)
