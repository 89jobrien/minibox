...
- Handler coverage in `daemonbox` must stay at or above the current baseline; the target
  is >= 80% function coverage (see `docs/STABILITY_CHECKLIST.md`).
...
The workspace currently has 19 crates across all tiers. Adding more crates before
...
2. Handler coverage >= 80% function coverage in `daemonbox/src/handler.rs`.
