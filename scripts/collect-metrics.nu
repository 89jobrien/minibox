...
def parse-test-output [raw: string] {
    ...
    let tests = ($events | where { |e| ($e | get type | default "") == "test" })
    let ok      = ($tests | where { |e| ($e | get event | default "") == "ok"      } | length)
    let failed  = ($tests | where { |e| ($e | get event | default "") == "failed"  } | length)
    let ignored = ($tests | where { |e| ($e | get event | default "") == "ignored" } | length)
    let total   = $ok + $failed + $ignored

    let failures = (
        $tests
        | where { |e| ($e | get event | default "") == "failed" }
        | get name
        | default []
    )
...
def load-bench-summary [] {
    ...
    $data.suites | each { |suite|
        let valid = ($suite.tests | where { |t| ($t | get stats | is-not-empty) })
        {
            suite:  $suite.name
            ...
            avg_us: (
                if ($valid | length) > 0 {
                    $valid | each { |t| $t | get stats.avg | default 0.0 } | math avg | math round --precision 1
                } else { null }
            )
        }
    }
