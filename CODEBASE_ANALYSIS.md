...
**Files**: `crates/minibox-lib/src/image/layer.rs:99-101`, `crates/minibox-lib/src/adapters/gke.rs:210-215`

The tar extraction code computes safe modes but never applies them -- the `tar` crate doesn't expose `set_mode()` before extraction, and **umask does NOT strip setuid bits**. The GKE `CopyFilesystem` adapter actively *re-applies* original permissions including setuid/setgid. Container images can deliver setuid root binaries.
...
**File**: `crates/minibox-lib/src/container/namespace.rs:17-63`
...
**File**: `crates/minibox-lib/src/image/layer.rs:150-162`
...
**File**: `crates/minibox-lib/src/container/filesystem.rs:114-125`
...
**Files**: `crates/minibox-lib/src/container/filesystem.rs:validate_layer_path`, `crates/minibox-lib/src/image/layer.rs:validate_tar_entry_path`, `crates/minibox-lib/src/image/layer.rs:verify_digest`
...
|---|---|---|---|
| 7 | Security | Container ID not validated before cgroup path construction | `crates/minibox-lib/src/container/cgroups.rs:45` |
| 8 | Security | `devtmpfs` mount exposes all host devices (`/dev/sda`, `/dev/mem`) | `crates/minibox-lib/src/container/filesystem.rs:217` |
| 9 | Security | `MAX_REQUEST_SIZE` enforced after full read into memory (DoS) | `crates/miniboxd/src/server.rs:68-96` |
| 10 | Security | FD leak window between `clone()` and `close_extra_fds()` | `crates/minibox-lib/src/container/process.rs:92-94` |
| 11 | Security | PID reuse race in `handle_stop` could kill wrong process | `crates/miniboxd/src/handler.rs:327-348` |
| 12 | Performance | Sequential layer downloads (5-layer image = 5x latency) | `crates/minibox-lib/src/image/registry.rs:351` |
| 13 | Performance | `spawn_blocking` contention between spawns and extraction | `crates/minibox-lib/src/adapters/runtime.rs:125` |
...
| 18 | Docs | `MAX_LAYER_SIZE` is 10GB but CLAUDE.md says 1GB | `crates/minibox-lib/src/image/registry.rs:43` |
...
| # | Category | Issue | File(s) |
|---|---|---|---|
| 19 | Architecture | Dual error hierarchy (`MiniboxError` + `DomainError`) with `anyhow` erasure | `crates/minibox-lib/src/error.rs`, `crates/minibox-lib/src/domain.rs` |
| 20 | Architecture | `AsAny` trait leaks test infrastructure into domain interface | `crates/minibox-lib/src/domain.rs:75-77` |
| 21 | Architecture | PID stored in both `ContainerRecord.pid` and `.info.pid` | `crates/miniboxd/src/state.rs:22-32` |
| 22 | Architecture | 5 speculative domain ports with no implementations | `crates/minibox-lib/src/domain/extensions.rs`, `crates/minibox-lib/src/domain/networking.rs` |
| 23 | Performance | `canonicalize(dest)` called per tar entry (~1400 syscalls/layer) | `crates/minibox-lib/src/image/layer.rs:146` |
| 24 | Performance | `CopyFilesystem` blocks async executor for ~600ms | `crates/minibox-lib/src/adapters/gke.rs:143` |
...
| 31 | Code | Cross-platform adapters compile on Linux without `#[cfg]` guards | `crates/minibox-lib/src/adapters/mod.rs` |
