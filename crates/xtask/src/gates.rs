...
pub fn test_conformance(sh: &Shell) -> Result<()> {
    ...
    cmd!(sh, "cargo build -p minibox-testsuite --bins")
        .run()
        .context("failed to build minibox-testsuite")?;

    // Run the full suite via `run-conformance` (fast, exits 1 on failure).
    let output = cmd!(sh, "cargo run -p minibox-testsuite --bin run-conformance")
        .output()
    ...
    let report_output = cmd!(sh, "cargo run -p minibox-testsuite --bin generate-report")
        .output()
