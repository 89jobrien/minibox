...
/// Verified workspace crate dependency graph (from Cargo.toml, 2026-03-30).
/// Layout: binaries → platform shims → core libs → leaf proc-macro.
/// Navigate nodes and read the detail panel for each crate's full dep list.
pub fn workspace_deps() -> Diagram {
    // Node IDs:
    //  0=miniboxd  1=minibox-cli  2=minibox-bench  3=dockerboxd
    //  4=macbox    5=winbox       6=daemonbox       7=minibox-client
    //  8=mbx       9=minibox-core 10=minibox-macros
    Diagram {
        name: "Workspace Deps",
        nodes: vec![
            Node {
                id: 0,
                label: "miniboxd",
                detail: "Unified daemon binary. Unconditional: daemonbox. \
                         Linux: mbx+minibox-core+nix. \
                         macOS: macbox. Windows: winbox.",
                kind: NodeKind::Artifact,
            },
            Node {
                id: 1,
                label: "minibox-cli",
                detail: "Platform-agnostic CLI binary. \
                         Deps: minibox-core + minibox-client. \
                         No direct mbx dependency.",
                kind: NodeKind::Artifact,
            },
            Node {
                id: 2,
                label: "mbx-bench",
                detail: "Benchmark harness binary. \
                         Deps: minibox-core + mbx.",
                kind: NodeKind::Artifact,
            },
            Node {
                id: 3,
                label: "dockerboxd",
                detail: "Docker API shim (axum HTTP over Unix socket). \
                         Translates Docker API → minibox protocol. \
                         Deps: minibox-client + minibox-core.",
                kind: NodeKind::Artifact,
            },
            Node {
                id: 4,
                label: "macbox",
                detail: "macOS daemon: Colima + vz adapters. \
                         Deps: daemonbox + minibox-core + mbx. \
                         Optional: objc2 + Virtualization.framework.",
                kind: NodeKind::Job,
            },
            Node {
                id: 5,
                label: "winbox",
                detail: "Windows daemon stub (HCS planned). \
                         Deps: daemonbox + mbx.",
                kind: NodeKind::Job,
            },
            Node {
                id: 6,
                label: "daemonbox",
                detail: "Platform-agnostic handler/state/server. \
                         Deps: minibox-core + mbx. \
                         Unix: nix for SO_PEERCRED auth.",
                kind: NodeKind::Job,
            },
            Node {
                id: 7,
                label: "mbx-client",
                detail: "Unix socket client: DaemonClient + \
                         DaemonResponseStream. \
                         Deps: minibox-core only.",
                kind: NodeKind::Job,
            },
            Node {
                id: 8,
                label: "mbx",
                detail: "Linux container primitives: namespaces, \
                         cgroups v2, overlay FS, image pull, adapters. \
                         Deps: minibox-core + minibox-macros + nix.",
                kind: NodeKind::Job,
            },
            Node {
                id: 9,
                label: "mbx-core",
                detail: "Cross-platform types: protocol, domain traits, \
                         image management, preflight. \
                         Deps: minibox-macros. Unix: nix.",
                kind: NodeKind::Job,
            },
            Node {
                id: 10,
                label: "mbx-macros",
                detail: "Proc-macro crate: as_any! and adapt! macros \
                         used by mbx and minibox-core. \
                         True leaf — no workspace deps.",
                kind: NodeKind::Command,
            },
        ],
        edges: vec![
        ...
        layout: vec![
            vec![0, 1, 2, 3], // binaries
            vec![4, 5, 6, 7], // platform shims + client
            vec![8, 9],       // core libs
            vec![10],         // leaf proc-macro
        ],
    }
}
