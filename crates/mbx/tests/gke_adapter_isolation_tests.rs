...
fn noop_limiter_create_with_various_ids() {
    ...
    for id in &["a", "container-123", "my-app-prod", "abc123def456"] {
        let path = limiter.create(id, &config).unwrap();
        assert_eq!(
            path,
            format!("noop:{}", id),
...
fn copy_filesystem_merges_single_layer() {
    ...
    let content = std::fs::read_to_string(merged.merged_dir.join("bin/sh")).unwrap();
...
fn copy_filesystem_later_layer_overwrites_earlier() {
    ...
    std::fs::create_dir_all(layer0.join("etc")).unwrap();
    std::fs::write(layer0.join("etc/os-release"), "layer0-content").unwrap();
    ...
    std::fs::write(layer1.join("etc/os-release"), "layer1-content").unwrap();
...
fn copy_filesystem_preserves_symlinks() {
    ...
    let layer = dir.path().join("layer");
    std::fs::create_dir_all(layer.join("bin")).unwrap();
    std::fs::write(layer.join("bin/busybox"), "fake-busybox-binary").unwrap();

    // Create a relative symlink sh -> busybox
    std::os::unix::fs::symlink("busybox", layer.join("bin/sh")).unwrap();
...
fn copy_filesystem_handles_directory_permissions() {
    ...
    #[cfg(not(unix))]
    {
        ...
        let dir = TempDir::new().unwrap();
        let layer = dir.path().join("layer");
        std::fs::create_dir_all(&layer.join("app")).unwrap();
...
fn adapters_can_be_used_simultaneously() {
    ...
    let layer1 = dir.path().join("layer1");
    std::fs::create_dir_all(layer1.join("bin")).unwrap();
    std::fs::write(layer1.join("bin/sh"), "fake").unwrap();
...
fn copy_filesystem_multiple_instances_independent() {
    ...
    std::fs::write(layer.join("etc/test"), "data").unwrap();
    ...
    fs1.cleanup(&c1).unwrap();
    assert!(!c1.exists());
    assert!(c2.exists());

    fs2.cleanup(&c2).unwrap();
    assert!(!c2.exists());
}
