...
//! - [`NoopNetwork`](linuxbox::adapters::NoopNetwork) adapter (from network/none.rs)
//! - [`NoopLimiter`](linuxbox::adapters::NoopLimiter) adapter (from gke.rs)
...
use linuxbox::adapters::{NoopLimiter, NoopNetwork};
...
/// `MockRegistry` from minibox-testers can be cast to `Arc<dyn ImageRegistry>`
...
/// `MockRuntime` from minibox-testers can be cast to `Arc<dyn ContainerRuntime>`
/// and downcast back to the concrete type.
#[tokio::test]
