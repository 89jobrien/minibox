...
use minibox_core::domain::{AsAny, ContainerId, ExecHandle, ExecRuntime, ExecSession, ExecSpec};
...
impl ExecRuntime for MockExecRuntime {
    ...
    async fn run_in_container(
        ...
        Ok(ExecSession {
            ...
            output: Box::new(TokioExecOutputStream::empty()),
        })
    }
}
...
mod tests {
    ...
    async fn records_call_and_returns_handle() {
        ...
        let session = mock.run_in_container(&id, spec.clone()).await.unwrap();
        assert_eq!(mock.call_count(), 1);
        assert_eq!(mock.last_container_id().unwrap(), id);
        assert_eq!(mock.last_spec().unwrap().cmd, spec.cmd);
        assert!(session.handle.id.contains("mock-exec"));
    }
    ...
    async fn with_failure_returns_error() {
        ...
        assert!(mock.run_in_container(&id, spec).await.is_err());
        assert_eq!(mock.call_count(), 1);
    }
    ...
    async fn call_count_increments_on_failure() {
        ...
        let _ = mock.run_in_container(&id, spec).await;
        assert_eq!(mock.call_count(), 1);
    }
}
