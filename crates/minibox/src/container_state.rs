...
//! to look up running container PIDs without depending on the daemon module.
//!
//! `DaemonState` in `minibox::daemon::state` implements this trait, breaking
//! what would otherwise be a circular dependency between adapters and daemon.

use std::sync::Arc;

/// Minimal state accessor needed by exec/commit adapters.
///
/// Implemented by `DaemonState` in `minibox::daemon::state`.
