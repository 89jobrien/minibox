...
impl ConformanceTest for RunInContainerReturnsHandle {
    ...
    fn run_sync(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let result = rt().block_on(mock.run_in_container(&container_id(), basic_spec(), tx));
...
impl ConformanceTest for RunInContainerIncrementsCount {
    ...
    fn run_sync(&self, ctx: &mut TestContext) -> TestResult {
        let mock = MockExecRuntime::new();
        let (tx, _rx) = tokio::sync::mpsc::channel(8);
        let _ = rt().block_on(mock.run_in_container(&container_id(), basic_spec(), tx));
...
impl ConformanceTest for RunInContainerRecordsSpec {
    ...
    fn run_sync(&self, ctx: &mut TestContext) -> TestResult {
        ...
        let _ = rt().block_on(mock.run_in_container(&container_id(), spec, tx));
