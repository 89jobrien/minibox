...
use minibox_core::{
    domain::{
        AsAny, ContainerRuntime, ContainerSpawnConfig, ImageMetadata, ImageRegistry,
...
async fn call_agent(vm: &Arc<VzVm>, req: &DaemonRequest) -> Result<Vec<DaemonResponse>> {
...
impl VzRegistry {
    ...
    pub const fn new(vm: Arc<VzVm>) -> Self {
        Self { vm }
    }
}
