...
mod imp {
    ...
    pub async fn connect_to_agent(vm: &Arc<VzVm>, max_attempts: u32) -> Result<UnixStream> {
        ...
        for attempt in 1..=max_attempts {
            match try_connect(Arc::clone(vm)).await {
                Ok(stream) => {
        ...
        Err(last_err).with_context(|| {
            format!(
                "vsock: failed to connect to agent on port {AGENT_PORT} after {max_attempts} attempt(s)"
            )
        })
    }
    ...
    ///
    /// All objc2/block2 FFI work — including the completion-handler poll loop
    /// — runs inside [`tokio::task::spawn_blocking`] rather than this `async
    /// fn`'s own body. objc2 types like `Retained<NSArray<..>>` and
    /// `block2::RcBlock` are not `Send`, so holding one across an `.await`
    /// point (the poll loop's sleep) makes the enclosing future `!Send`,
    /// which breaks every `async_trait` method that calls through here (see
    /// `VzVm`'s own `unsafe impl Send`/`Sync` for the same reasoning applied
    /// at the VM-handle level). Only plain `Send` data — the final `RawFd` —
    /// crosses back out of the blocking closure.
    async fn try_connect(vm: Arc<VzVm>) -> Result<UnixStream> {
        let raw_fd = tokio::task::spawn_blocking(move || -> Result<RawFd> {
            ...
            let virtio_retained = base.downcast::<VZVirtioSocketDevice>().map_err(|_| {
                anyhow::anyhow!("vsock: socket device is not a VZVirtioSocketDevice")
            })?;
            ...
            let block = block2::RcBlock::new(
                move |conn: *mut objc2_virtualization::VZVirtioSocketConnection,
                    ...
                    if raw_fd < 0 {
                        *guard = Some(Err(anyhow::anyhow!(
                            "VZVirtioSocketConnection returned fd={raw_fd}, connection closed"
                        )));
                        return;
                    }
            ...
            // virtio_dev are valid for the duration of this call.
            struct ConnectCtx<'a> {
            ...
            unsafe {
                dispatch_sync_f(
                    ...
                    (&raw mut connect_ctx).cast::<std::ffi::c_void>(),
                    connect_trampoline,
                );
            }

            // Poll for the result.  VZ fires the completion handler quickly but we
            // must not busy-loop indefinitely. This is a synchronous poll (not an
            // async one) since we're inside spawn_blocking — see the fn doc comment.
            ...
            loop {
                {
                    ...
                    if let Some(ref outcome) = *guard {
                        ...
                        // Set the fd to non-blocking so Tokio can poll it once we're
                ...
                if Instant::now() > deadline {
                    bail!(
                        "vsock: timed out waiting for VZ completion handler on port {AGENT_PORT}"
                    );
                }
        ...
        .await
        .map_err(|e| anyhow::anyhow!("vsock: try_connect spawn_blocking join error: {e}"))??;

        // SAFETY: raw_fd is a valid non-blocking socket fd that we own
        // exclusively (obtained via dup inside the blocking closure above).
        // We transfer ownership to std::os::unix::net::UnixStream which will
        // close it on drop.  Then we wrap with the Tokio async type.
        let std_stream = unsafe {
            use std::os::unix::io::FromRawFd;
            std::os::unix::net::UnixStream::from_raw_fd(raw_fd)
        };
        UnixStream::from_std(std_stream).context("UnixStream::from_std for vsock fd")
    }
}
...
mod imp {
    ...
    pub async fn connect_to_agent(_vm: &Arc<VzVm>, _max_attempts: u32) -> Result<UnixStream> {
