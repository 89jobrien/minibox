...
mod imp {
    ...
    /// Shared slot the VZ completion-handler block writes its boot outcome into.
    ///
    /// Polled by the caller from a worker thread. `Ok(())` on success,
    /// `Err(String)` holding the VZ-reported error description on failure.
    pub type StartSignal = Arc<Mutex<Option<Result<(), String>>>>;
    ...
    // qual:allow(complexity) reason: "Retained<VZVirtualMachine> is not provably Send by clippy's static analysis, but the SAFETY comment above documents why moving it across threads is sound here"
    ...
    impl VzVm {
        ...
        pub fn prepare_on_main_queue(config: VzVmConfig) -> Result<(Self, StartSignal)> {
            ...
            let start_signal: StartSignal = Arc::new(Mutex::new(None));
            ...
            let block = block2::RcBlock::new(move |error: *mut objc2_foundation::NSError| {
                ...
                if error.is_null() {
                    ...
                    *guard = Some(Err(format!("{description} (domain={domain} code={code})")));
                }
            });
            ...
            Ok((Self { vm, config }, start_signal))
        }
