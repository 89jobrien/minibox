...
const fn is_terminal(resp: &DaemonResponse, is_ephemeral_run: bool) -> bool {
    // ContainerCreated: the canonical DaemonResponse::is_terminal() always
    // treats this as non-terminal (the server distinguishes ephemeral vs.
    // non-ephemeral by dropping its response channel, not by response
    // content). The vsock proxy has no channel to drop, so it must decide
    // from the request's `ephemeral` flag instead — non-ephemeral runs end
    // here; ephemeral runs continue on to stream ContainerOutput chunks.
    if let DaemonResponse::ContainerCreated { .. } = resp {
        return !is_ephemeral_run;
    }
    resp.is_terminal()
}
