...
/// - `ContainerStopped` (end of ephemeral run)
/// - `ContainerCreated` only for non-ephemeral runs (fire-and-forget)
/// - `Error` (request failed)
/// - `Success` (generic success)
/// - `ContainerList` (response to List request)
///
/// Non-terminal responses:
/// - `ContainerCreated` (ephemeral runs continue with ContainerOutput)
/// - `ContainerOutput` (can repeat multiple times)
///
/// For ephemeral runs (streaming), the response sequence is:
/// ContainerCreated (non-terminal) -> [ContainerOutput (non-terminal)]* -> ContainerStopped (terminal)
...
mod tests {
    ...
    async fn proxy_closes_on_eof_without_terminal() {
        ...
        let _handle = tokio::spawn(async move {
            use tokio::io::AsyncWriteExt;
            // Send a ContainerCreated (non-terminal)
