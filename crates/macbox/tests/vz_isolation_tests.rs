...
fn main() {
    ...
    std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            unsafe {
                let code = Box::into_raw(Box::new(i32::from(failed)));
                dispatch_async_f(&raw const _dispatch_main_q, code.cast(), exit_trampoline);
            }
        })
...
mod suite {
    ...
    /// A named async test case, boxed so heterogeneous test futures can share
    /// one array despite each having a distinct concrete future type.
    type TestCase = (
        &'static str,
