...
pub(crate) fn resolve_expr(expr: &str, state: &WorkflowState) -> anyhow::Result<String> {
    use anyhow::Context as _;
...
pub(crate) fn resolve_output_ref(expr: &str, state: &WorkflowState) -> anyhow::Result<String> {
    let expr = expr.trim();
